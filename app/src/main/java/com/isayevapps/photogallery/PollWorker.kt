...
import com.isayevapps.photogallery.ui.MainActivity
...
class PollWorker(
    ...
    override suspend fun doWork(): Result {
        ...
        if (query.isEmpty()) {
            Log.i("MyTaggg", "No saved query, finishing early.")
        ...
        return try {
            ...
            if (items.isNotEmpty()) {
                ...
                if (newResultId == lastId) {
                    Log.i("MyTaggg", "Still have the same result: $newResultId")
                } else {
                    Log.i("MyTaggg", "Got a new result: $newResultId")
            ...
            Log.e("MyTaggg", "Background update failed", ex)
            Result.failure()
        }
    }
